...
class CarState(CarStateBase, MadsCarState):
  ...
  def update(self, can_parsers) -> tuple[structs.CarState, structs.CarStateSP]:
    ...
    elif self.CP.flags & VolkswagenFlags.MQBEVO:
      return self.update_mqbevo(pt_cp, main_cp, cam_cp, ext_cp)
  ...
  def update_mqbevo(self, pt_cp, main_cp, cam_cp, ext_cp) -> tuple[structs.CarState, structs.CarStateSP]:
    ret = structs.CarState()
    ret_sp = structs.CarStateSP()
    ...
    ret.parkingBrake = pt_cp.vl["ESC_50"]["EPB_Status"] in (1, 4) # both signals are seen, maybe similar to MEB: use candidate for all MEB plattforms and years
  ...
  def get_can_parsers(CP, CP_SP):
    ...
    elif CP.flags & VolkswagenFlags.MQBEVO:
      return CarState.get_can_parsers_mqbevo(CP)
