...
def apply_std_curvature_limits(apply_curvature: float, apply_curvature_last: float, v_ego: float, curvature: float,
                               override: bool,
                               steer_step: int, lat_active: bool, limits: CurvatureSteeringLimits, roll=None,
                               yaw_rate=None) -> float:
  ...
  if roll:  # true roll asymmetrically (need roll for panda...) -> I've sent this via custom CAN message, but prefering sensor data channel to panda with exact time frame assignment
    ...
    assert False  # not allowed
    min_curvature, max_curvature = get_max_curvature_yaw_rate(yaw_rate, apply_curvature, apply_curvature_last, v_ego,
                                                              steer_step, override)
